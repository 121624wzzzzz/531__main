...
\subsection{S1--S10 模型变体定义}

为系统研究 tied embedding/unembedding 矩阵的结构化松绑方式及其对语言模型训练的影响，
本文在相同的 Transformer 主干上构造十种 embedding-head 变体，记为 S1--S10。
...
本文首先将 S1--S7 统一理解为对 tied 矩阵 $W$ 的不同松绑方式，并进一步在 S8--S10
中考察 untied 输入 embedding 矩阵 $E$ 上的三种松绑消融。具体而言，不同变体的核心
...
\paragraph{S4: 输入侧常规 LoRA 加性松绑。}
S4 在输入 embedding 矩阵上做常规加性 LoRA 松绑。输入侧有效矩阵为
\begin{equation}
...
\paragraph{S5: 输出侧常规 LoRA 加性松绑。}
S5 与 S4 对称，但 LoRA 加在输出 projection 上。输入侧保持 tied：
\begin{equation}
    E_{\mathrm{eff}} = W,
\end{equation}
因此
...
S5 的含义是输入侧仍然 tied，输出侧通过常规 LoRA 获得额外自由度。S4 和 S5
构成一组输入侧加性 LoRA 与输出侧加性 LoRA 的结构对照。

\paragraph{S6: 输入侧乘性低秩松绑。}
S6 是输入侧的乘性松绑方式。由于语言模型中通常有 $V \gg d$，直接在
$V \times d$ 的 embedding 矩阵上做 LoRA 会引入与词表大小 $V$ 相关的参数。
...
S6 的含义是不额外学习一个 $V \times r$ 的词表侧 LoRA，而是在 hidden dimension
...
S7 的含义是不在词表维度上给输出矩阵加 LoRA，而是在输出 projection 的隐藏维度方向
...
\paragraph{S9: Untied Embedding + Additive LoRA，独立输入矩阵上的常规 LoRA 松绑。}
S9 同样以 S2 的 full untied 结构为基础，但在独立输入 embedding 矩阵 $E$ 上加入
常规加性 LoRA 修正。输入侧有效矩阵为
\begin{equation}
...
S9 用于考察常规词表维度 LoRA 是否能够在 full untied 的输入 embedding 上带来额外收益。
与 S4 不同，S9 的低秩加性修正不是对共享矩阵 $W$ 进行松绑，而是对已经独立学习的
输入矩阵 $E$ 进行进一步重参数化。

\paragraph{S10: Untied Embedding + Multiplicative LoRA，独立输入矩阵上的乘性低秩松绑。}
...
S10 用于检验在 full untied 的输入 embedding 上，隐藏维度乘性松绑是否比词表维度加性 LoRA
更具参数效率。其额外参数量为 $2dr$，不随词表大小 $V$ 增长。与 S6 不同，S10 的乘性
低秩变换作用在独立输入矩阵 $E$ 上，而不是共享矩阵 $W$ 上。

\subsection{十二个模型的统一对比}

\begin{table}[htbp]
\centering
\caption{S1--S12 模型变体的有效输入矩阵与有效输出矩阵。}
\label{tab:s1_s12_variants}
\begin{tabular}{llll}
\toprule
...
S4 & 输入侧常规 LoRA 加性松绑 & $W+A_{\mathrm{in}}B_{\mathrm{in}}$ & $W$ \\
S5 & 输出侧常规 LoRA 加性松绑 & $W$ & $W+A_{\mathrm{out}}B_{\mathrm{out}}$ \\
...
S9 & untied 输入侧常规 LoRA 加性松绑 & $E+A_EB_E$ & $U$ \\
S10 & untied 输入侧乘性低秩松绑 & $E(I_d+P_EQ_E)$ & $U$ \\
S11 & 输出侧隐藏维全局平移松绑 & $W$ & $W,\ h_t\mapsto h_t-\beta_{\mathrm{out}}$ \\
S12 & 输入侧全局平移 + 乘性低秩松绑 & $W(I_d+P_{\mathrm{in}}Q_{\mathrm{in}})-\mathbf{1}\beta^\top$ & $W$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{十个模型的逻辑关系}
...
单向量平移松绑、常规 LoRA 加性松绑，以及隐藏维度上的乘性松绑；同时，我们在 untied
输入 embedding 矩阵 $E$ 上加入同样三种形式，作为输入层松绑的消融对照。
...
对于常规 LoRA 松绑，我们分别在输入侧或输出侧引入低秩加性矩阵：
\begin{equation}
    E_{\mathrm{eff}} = W+A_{\mathrm{in}}B_{\mathrm{in}},
\end{equation}
或
